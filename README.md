...
The `.battle-layout` class keeps the local camera and signs in the left half and the opponent in the right half. The bottom is a shared arena: Sasuke always faces right from the left, Naruto always faces left from the right. Room host uses Sasuke and guest uses Naruto, independent of which camera is on the local left. `BattleSprites` in `src/battle-sprites.js` is a standalone renderer that Single Player can reuse: construct it with the arena element, call `start()`, `attack('sasuke'|'naruto')`, and `stop()` at teardown. It alternates the first two sheet cells at idle and plays all six cells for every attack, moving toward the center. Animation and audio do not gate hand recognition.
