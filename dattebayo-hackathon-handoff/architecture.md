...
1. `handoff-references/pixel-style-primary.png` is the primary art-direction
   reference: chunky, low-resolution pixel art with hard nearest-neighbor
   edges. It replaces any earlier assumption of smooth or high-resolution
   shaded illustration.
2. `street-fighter-layout-reference.png`, `gold-buttons-reference.png`,
   `valley-environment-reference.png`, and `title-logo-reference.png` define
   the approved composition, controls, environment, and title treatment.
3. The existing `battle-layout-reference.png`,
   `naruto-rasengan-left-facing-reference.png`,
   `sasuke-chidori-right-facing-reference.png`, and
   `secret-ending-composition-reference.png` are secondary pose and
   composition references only. They never override the chunky pixel style
   and are not sprite sheets, atlases, or licensed source art.
...
separate, read-only migration reference for the pinned CV modules, tests, and
credited audio assets. Prefer it over
...
- The primary look is chunky low-resolution pixel art. Author sprites on a
  logical `64x64` grid and export them at `256x256` with integer `4x`
  nearest-neighbor scaling; do not generate high-resolution shaded
  illustrations and downsample them.
...
- Trim bounds, preserve the feet pivot, and use nearest-neighbor pixel sampling.
...
- The primary reference is `pixel-style-primary.png`; the approved layout,
  controls, environment, and title references are
  `street-fighter-layout-reference.png`, `gold-buttons-reference.png`,
  `valley-environment-reference.png`, and `title-logo-reference.png`.
- The existing `battle-layout-reference.png`,
  `naruto-rasengan-left-facing-reference.png`,
  `sasuke-chidori-right-facing-reference.png`, and
  `secret-ending-composition-reference.png` are pose/composition samples only.
- All references live in `handoff-references/`, are read-only visual samples,
  and are not atlases or license grants.

Copy-paste generation prompts:

- Fighter: “CHUNKY low-resolution pixel-art side-view game sprite, authored on a
  logical 64x64 grid and exported at 256x256 with integer 4x nearest-neighbor
  pixels; [Sasuke in a gray open-collar outfit with purple rope/Naruto in a
  black-orange outfit], readable idle/cast/dash/hit/victory/defeat poses, fixed
  [right/left] facing, feet on a shared baseline, transparent background, no
  smooth shading, no checkerboard, no text, no copied anime frame.”
- Background: “CHUNKY low-resolution pixel-art, logical 480x180 grid exported
  at 1920x720 with integer 4x nearest-neighbor pixels; original wide side-on
  Valley of the End with giant statues and a muted waterfall in the rear,
  shallow reflective water surface, ripples and reflections, no platforms or
  separate floor, no characters, no UI, no text.”
- Secret ending: “CHUNKY low-resolution pixel-art, logical 480x180 grid exported
  at 1920x720 with integer 4x nearest-neighbor pixels; original non-graphic
  ending composition: exhausted Sasuke and Naruto collapsed on rocks after a
  Chidori/Rasengan clash, wrapped or obscured hands and forearms, dramatic smoke
  and dusk light, no blood, gore, exposed wounds, or text.”
