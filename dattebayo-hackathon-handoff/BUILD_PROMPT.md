...
for the pinned, read-only migration bundle and credits. The latest visual references are
`pixel-style-primary.png`, `gold-buttons-reference.png`,
`valley-environment-reference.png`, `title-logo-reference.png`, and
`street-fighter-layout-reference.png`; the four earlier pose/layout/ending
references are secondary only and never define the art style.

Use Vite + vanilla TypeScript + Canvas2D + DOM HUD. Start with a functioning
greybox: chunky low-resolution pixel art, authored on logical `64x64` sprite
grids and exported at `256x256` with integer `4x` nearest-neighbor scaling.
Keep Sasuke left facing right in a gray open-collar/purple-rope outfit and
...
Generate original CHUNKY low-resolution pixel-art runtime art using the exact
manifest and prompts in `architecture.md`: fixed-facing Sasuke/Naruto idle,
...
sprites on logical `64x64` grids and export `256x256` with integer `4x`
nearest-neighbor scaling, with no smooth high-resolution shading. Use relative
runtime paths, real alpha, trimmed frames, stable feet pivots, and preview
validation. All files in `handoff-references/` are visual samples only, not
